...
/**
 * Safely calls argument-less methods while preventing Null Pointer Exceptions.
 * It follows a similar fluent style as the Optional class,
 * however the {@param transformer} function callback is only called when the input is NOT NULL.
 * The Optional class does not check for null pointers between subsequent Optional.map calls.
 */
...
public class SafeGet<T, R> {
  ...
  /**
   * If the value contained in this instance is NOT NULL,
   * the transformer will be called and a new SafeGet instance will be returned for further processing
   * @param newTransformer converts the contained {@link SafeGet#value} to a different the output type
