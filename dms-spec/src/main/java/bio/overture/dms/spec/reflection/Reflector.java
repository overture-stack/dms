package bio.overture.dms.spec.reflection;
...
import static bio.overture.dms.spec.reflection.ReflectorException.checkReflection;
...
public class Reflector {
  ...
  @SneakyThrows
  public Object getFieldValue(@NonNull Class<?> objClass, @NonNull Object obj,
      @NonNull Field f){
    checkReflection(obj.getClass() == objClass,
        "The objClass parameter '%s' and obj parameter's class '%s' do not match",
        objClass.getName(), obj.getClass().getName() );
  ...
  public Method getGetterMethod(@NonNull Class<?> objClass, @NonNull Field f){
    ...
    checkReflection(result.isPresent(),"Could not find getter method for field '%s' for object type '%s'",
        f.getName(), objClass.getName());
    return result.get();
  }
