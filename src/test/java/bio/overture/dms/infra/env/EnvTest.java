...
import bio.overture.dms.infra.config.EnvConfig;
...
public class EnvTest {
  ...
  private static final EnvProcessor ENV_PROCESSOR;

  static {
    val c = new EnvConfig();
    ENV_PROCESSOR = c.buildEnvProcessor(c.buildReflector(c.buildReflections()));
  }
  ...
  public void generateEnvMap_NullCustomChildren_Success() {
    val person2 = Person.builder().lastName("Doe").build();
    val employee =
        Employee.builder()
            .person(null)
            .person2(person2)
    ...
    Assertions.assertTrue(map.containsKey(COMPANY));
    Assertions.assertTrue(map.containsKey(PERSON2_LASTNAME));
  ...
  public void dumpEnvs_NonNested_Success() {
    ...
    Stream.of(LASTNAME, AGE, MALE, HEIGHT, WEIGHT, MYCHAR, NUM_CHILDREN, HAS_HOUSE, INCOME)
        .forEach(
            x ->
                assertTrue(
                    envVars.contains(x),
                    "Expected env variable '%s' does not exist in the env vars dump",
                    x));
  }
  ...
  public void dumpEnvs_Nested_Success() {
    ...
    val totalExpectedEnvVariables =
        numPersonFieldsPerEmployee * fieldsPerPerson + numEmployeeFields;
    assertEquals(totalExpectedEnvVariables, envVars.size());
    Assertions.assertTrue(envVars.contains(TEST_PERSON2_AGE));
    Assertions.assertTrue(envVars.contains(TEST_PERSON2_HAS_HOUSE));
    Assertions.assertTrue(envVars.contains(TEST_PERSON2_WEIGHT));
  }
  ...
  public void generateEnvMap_Employee_Success() {
    ...
    expectedEnvVariables.forEach(
        x ->
            assertTrue(
                map.containsKey(x),
                "Expected env variable '%s' does not exist in the generated map",
                x));
  ...
  private static void assertEnvProcessingException(
      @Nullable Object obj, @NonNull String containingTextInMessage) {
    assertExceptionThrown(
        () -> ENV_PROCESSOR.generateEnvMap(obj),
        EnvProcessingException.class,
