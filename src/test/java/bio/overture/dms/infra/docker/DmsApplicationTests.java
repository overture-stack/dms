...
class DmsApplicationTests {
  ...
  @Autowired private DmsDeploymentService dmsDeploymentService;

  @Autowired private DockerService dockerService;

  @Autowired private DockerClient dockerClient;

  @Autowired private DockerComposeClient dockerComposeClient;

  @Disabled
  @Test
  @SneakyThrows
  public void teste() {
    val egoSpec = EgoSpec.builder().host("https://example.org").build();
    val dmsSpec = DmsSpec.builder().version("1.0.0").ego(egoSpec).build();
