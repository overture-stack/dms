...
public class ComposeInfoService {
   ...
   *
   * @param serviceName The name of the container to inspect
   * @return Optional deploy info
   */
  public Optional<ComposeServiceInfo> readComposeServiceInfo(@NonNull String serviceName) {
    return dockerService.inspectContainerByName(serviceName).map(x -> doit(x, serviceName));
  }
    ...
    return SafeGet.of(r, InspectContainerResponse::getMounts)
        .map(
            mounts ->
                mounts.stream()
                    .collect(
                        toUnmodifiableMap(Mount::getSource, x -> x.getDestination().getPath())))
        .get();
  }
