...
public class EgoUiDeployTask implements PreDeployTask {
  ...
  private static final RandomGenerator RANDOM_GENERATOR = createRandomGenerator(EgoUiDeployTask.class.getSimpleName());
  ...
  public void executePreDeployTask(@NonNull DmsConfig dmsConfig) {
    ...
    if (result.isPresent()){
      val app = result.get();
      terminal.printStatusLn( "[%s]: EGO Application '%s' already exists, skipping creation.",
          getClass().getSimpleName(), app.getName());
    } else {
        val app = egoClient.createApplication(
            CreateApplicationRequest.builder()
                ...
                .build());
      terminal.printStatusLn( "[%s]: Created EGO Application '%s' successfully",
          getClass().getSimpleName(), app.getName());
    }
  }
