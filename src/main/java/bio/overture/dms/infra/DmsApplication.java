...
public class DmsApplication {
  ...
  public static void main(String[] args) {
    ...
    val envProcessor =
        EnvProcessor.createEnvProcessor(Reflector.createReflector(Reflections.collect()));
    ...
    docker.pullImage(DockerImage.builder().repositoryName(postgreRepo).tag(postgresTag).build());
    val resource = FileUtils.readResourcePath("/assets/ego-init/init.sql");
    ...
    val egoTag = "3.1.0";
    docker.pullImage(DockerImage.builder().repositoryName(egoRepo).tag(egoTag).build());
    val egoContainer =
        dockerClient
