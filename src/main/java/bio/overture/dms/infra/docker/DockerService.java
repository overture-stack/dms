...
public class DockerService {
  ...
  public String createVolumeWithAssets() {
    ...
    val containerId =
        client
            ...
            .getId();
    val rootAssetsPath =
        FileUtils.readResourcePath("/assets").getFile().toPath().toAbsolutePath().toString();
    client
  ...
  public String createContainer(
      @NonNull String networkName, @Nullable String assetVolumeName, @NonNull DCService s) {
    checkArgument(
  ...
  public String createContainer(@NonNull DockerContainer<?> dockerContainer) {
    val containerId =
        client
            .createContainerCmd(dockerContainer.getDockerImage().getFullName())
    ...
    client.connectToNetworkCmd().withContainerId(containerId).withNetworkId(networkId).exec();
    return containerId;
  }
  ...
  // TODO: first do concurrent pulling of images. When EGO pulling is done, then continue with EGO
  // deployement
  @SneakyThrows
  public void pullImage(@NonNull DockerImage dockerImage) {
    client
        .pullImageCmd(dockerImage.getName())
        .withTag(dockerImage.getTag())
        .start()
        .awaitCompletion();
  }
