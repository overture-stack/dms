...
public class DockerComposeClient {
  ...
  /** Constants */
  private static final Mount DOCKER_SOCK_MOUNT =
      new Mount()
          .withType(BIND)
  ...
  private static final Mount DOCKER_EXEC_MOUNT =
      new Mount()
          .withType(BIND)
  ...
  private static final HostConfig HOST_CONFIG =
      HostConfig.newHostConfig().withMounts(List.of(DOCKER_SOCK_MOUNT, DOCKER_EXEC_MOUNT));
  ...
  public DCResponse runCommand(@NonNull File dockerComposeFile, @NonNull String command)
  ...
  private static boolean matchImageName(Image image, String imageName) {
    return image.getId().replaceAll("^\\S+:", "").startsWith(imageName)
        || matchTag(image.getRepoTags(), imageName);
  }
  ...
  private void removeExistingContainer() {
    dockerClient
        ...
        .stream()
        .findFirst()
        .ifPresent(x -> dockerClient.removeContainerCmd(x.getId()).withForce(true).exec());
  }
  ...
  private String createContainer(String command) {
    ...
    return dockerClient
        .createContainerCmd(dockerComposeImageName)
  ...
  private DCResponse getContainerLogs(String containerId) throws InterruptedException {
    ...
    try {
      ...
      /**
       * NOTE: for some reason, false stderrs are reported via the docker logs command for the
       * docker-compose container. It is not clear why the output of, `docker-compose kill`, shows
       * up as STDERR when docker logs is run, even though there were no errors. Because of this, we
       * do manual error matching via the `ERROR:` keyword. This is not ideal, but is better than
       * what there was.
       */
      val actuallyHasError =
          callback.hasError()
              && stream(callback.getStderr().split("\n")).anyMatch(x -> x.contains("ERROR:"));
      if (true) {
