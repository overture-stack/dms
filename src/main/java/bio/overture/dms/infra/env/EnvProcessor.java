...
import static bio.overture.dms.infra.env.EnvProcessingException.checkEnvProcessing;
import static bio.overture.dms.infra.reflection.Reflector.isClassPublic;
...
public class EnvProcessor {
  ...
  /**
   * Generates an environment variable hashmap by recursively processing field names. If the root
   * object is defined by the Person class below
   *
   * <pre>{@code}
   * public class Person {
   ...
   * </pre>
   *
   * The {@link EnvVariable} annotation explicitly defines the environment variable for the
   * associated field, instead of resolving it using the default convention.
   *
   * <pre>
   *
   *  </pre>
   ...
   * @throws EnvProcessingException when a env variable collision occurs or when a getter method is
   *     not found
   */
  ...
  /**
   * Similar to the {@link EnvProcessor#generateEnvMap(Object)} method, this method processes an
   * input object and extracts all possible environment variables. This is useful for testing how an
   * object will render.
   *
   * <pre>
   * </pre>
   *
   * Note: For objects with children that are also processable, the children MUST NOT be null. If
   * they are null, the environment variable name will NOT be included in the final list.
   *
   * <pre>
   * </pre>
   *
  ...
  private Map<String, String> internalGenerateEnvMap(
      @NonNull Class<?> objClass, @Nullable Object obj, EnvFieldVisitor envFieldVisitor) {
    checkEnvProcessing(
        isClassPublic(objClass),
  ...
  private void processObject(
      @NonNull Map<String, String> envMap,
      @Nullable String inputPrefix,
      @NonNull EnvFieldVisitor envFieldVisitor,
      @NonNull Class<?> objClass,
      @Nullable Object obj) {
    stream(objClass.getDeclaredFields())
        .filter(x -> !isStatic(x.getModifiers()))
        .forEachOrdered(
            field -> processField(envMap, objClass, obj, inputPrefix, field, envFieldVisitor));
  }
  ...
  private void processField(
    ...
    EnvProcessingException.checkEnvProcessing(
        !envMap.containsKey(envName),
  ...
  private static String resolveEnvName(@Nullable String inputPrefix, Field f) {
    val baseName =
        f.isAnnotationPresent(EnvVariable.class)
            ? f.getAnnotation(EnvVariable.class).value()
            : convertFieldNameToEnvName(f);
