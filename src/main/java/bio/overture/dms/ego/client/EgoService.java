...
/**
 * Convenience service that simplifies popular interaction with a running ego server,
 * using the EgoClient and (mostly) idempotent methods.
 */
...
public class EgoService {
  ...
  /**
   * For an existing group and policy, creates a GroupPermission only if a permission was
   * not already created. * If a permission already exists, nothing happens.
   ...
   */
  public void createGroupPermission(@NonNull String groupName, @NonNull String policyName,
    ...
    checkEgoClient(groupResult.isPresent(),
        "Cannot create group permission since the group '%s' does not exist", groupName);
    checkEgoClient(policyResult.isPresent(),
        "Cannot create group permission since the policy '%s' does not exist", policyName);
    ...
    findGroupPermissionByPolicyName(groupId, policyName)
        .ifPresentOrElse(x -> {},
            () -> client.createGroupPermission(groupId, List.of(PermissionRequest.builder()
   ...
   * {@code groupName} with the {@code groupRequest}. Note if the {@code groupRequest} has a different name field,
   * then the group will be renamed to that new name.
   * @param groupName name of group to create or update
   * @param groupRequest body of the group to create or update. In the case of creation, the {\@param groupName} must equal the {@param groupRequest.name} field
   * @return group
   */
  public EgoGroup saveGroup(@NonNull String groupName, @NonNull GroupRequest groupRequest){
    ...
    if (result.isPresent()){
      ...
      checkState(groupName.equals(groupRequest.getName()),
          "Cannot create Group with name '%s' since request does not contain the same name", groupName);
  ...
  public EgoPolicy savePolicy(@NonNull String policyName, @NonNull PolicyRequest r){
    ...
    if (result.isPresent()){
      ...
      checkState(policyName.equals(r.getName()),
          "Cannot create Policy with name '%s' since request does not contain the same name", policyName);
   ...
   */
  public EgoApplication saveApplication(@NonNull String applicationName, @NonNull ApplicationRequest r) {
    ...
    if (result.isPresent()){
      ...
      checkState(applicationName.equals(r.getName()),
  ...
  public Optional<GroupPermission> findGroupPermissionByPolicyName(@NonNull String groupId, @NonNull String policyName){
    val listRequest = ListGroupPermissionsRequest.builder()
        .id(groupId)
        .offset(0)
        .limit(30)
        .build();
  ...
  public Optional<EgoPolicy> findPolicyByName(@NonNull String policyName) {
    ...
    while(true){
      ...
      val result = page.getResultSet().stream()
          .filter(x -> x.getName().equals(policyName))
          .findFirst();
      if (result.isPresent()){
        return result;
      } else {
  ...
  private static boolean isUpdatedNeeded(@NonNull EgoApplication egoApplication, @NonNull ApplicationRequest r){
