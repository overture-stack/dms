        ...
        stage('Build & Publish Develop') {
            ...
            steps {
                container('docker') {
                    ...
                    sh "docker build ./nginx/path-based --network=host -f ./nginx/path-based/Dockerfile . -t ${dockerOrg}/${dmsGatewayRepo}:edge -t ${dockerOrg}/${dmsGatewayRepo}:${version}-${commit}"
                    sh "docker push ${dockerOrg}/${dmsRepo}:${version}-${commit}"
                    sh "docker push ${dockerOrg}/${dmsRepo}:edge"
                    sh "docker push ${dockerOrg}/${dmsGatewayRepo}:${version}-${commit}"
        ...
        stage('Release & tag') {
          ...
          steps {
                container('docker') {
                    ...
                    sh "docker build ./nginx/path-based --network=host -f ./nginx/path-based/Dockerfile . -t ${dockerOrg}/${dmsGatewayRepo}:latest -t ${dockerOrg}/${dmsGatewayRepo}:${version}-${commit}"
